...
|---|---|---|---|
| 1 | Log out | **Greyed owl**, no badge, tooltip "Quotewise — log in to capture quotes" | SC-001 |
| 2 | Log in, open a **new** tweet | Color owl, `★` blue, "New quote — not in Quotewise yet" | SC-002 |
| 3 | Submit it, revisit | `✓` green, "Already in your collection" | SC-002 |
| 4 | Open a known **exact** duplicate tweet | `=` orange, "Exact match already in Quotewise" | SC-002/006 |
| 5 | Open a **paraphrase** (near-same) | `~` purple, "Similar version already in Quotewise" | SC-002/006 |
| 6 | Open a **misattributed** quote | `⚠` vermillion, "Heads up — attributed to someone else…" | SC-002 |
| 7 | While a check is in flight | static `●` sky (no animation) | FR-013 |
| 8 | Expire the session on a duplicate tweet | `!` vermillion (Error **wins** over the dup badge) | SC-003 |
| 9 | Switch to a non-tweet tab | no quote badge leaks; ambient owl only | SC-007 |
...
- **Glyph legibility at 1× and 2×** (HiDPI): confirm `=` vs `~` are not confusable and `⚠` does not
  render as a color emoji (Decision D9). If it does, substitute within the same shape family.
