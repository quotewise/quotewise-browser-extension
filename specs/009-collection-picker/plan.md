...
Technical approach: a new pure seed/reconcile helper + a `CollectionPicker` UI component in the existing Shadow-DOM overlay; one new idempotent backend endpoint (`POST /v1/collections/{id}/quotes/`) called once per target collection (best-effort, per-collection feedback); a `member_collections` field added to the duplicate-check response; collection list preloaded into `storage.local` (rebuildable, ~5 min TTL) and the last-used set persisted in `storage.sync` (written once per completed add, only when changed — well inside the 120/min write limit). No new npm dependencies and no new manifest permissions.
...
- `chrome.storage.local` — collection-list cache `{ collections, default_collection_id, ts }` with ~5 min TTL, mirroring `preloadedOriginator`/`preloadedDuplicateCheck`. Rebuildable disposable cache (Article V).
...
**Performance Goals**: Picker opens with no perceptible spinner on a cache hit (preloaded list); preload runs alongside existing originator/duplicate preloads.

**Constraints**: Single-file SW/content bundles (`splitChunks: false`); no new permissions/dependencies (Article III); `storage.sync` write-rate and 8 KB/item limits respected; collection-list cache and last-used set wiped on logout/private/clear-data (Article II.2).
...
- **II — Privacy & Data Minimization**: PASS. The collection-list preload is an authenticated GET of the user's own account data — it sends no tweet identifiers, no quote text, and performs no write, so it sits outside the per-tweet pre-action egress concern; it is still bound to the existing preload-disable setting (FR-023). Cache + last-used set wiped on logout/private/clear-data (FR-024).
...
│   └── components/
│       ├── collection-picker.ts            # NEW — multi-select list, read-only "Already in", empty state
...
├── background/
│   ├── service-worker.ts                   # preload collections (storage.local, TTL); badge after add
│   ├── api-handler.ts                      # route ADD_QUOTE_TO_COLLECTION; serve cached LIST_COLLECTIONS
...
**Structure Decision**: Single existing extension project. The only new files are one UI component (`collection-picker.ts`), one pure helper (`collection-seed.ts`), and their tests; everything else extends existing modules following their current patterns (component composition under `overlay-bar.ts`, message routing in `api-handler.ts`, preload/cache mirroring the existing originator/duplicate preloads).
