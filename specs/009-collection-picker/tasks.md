...
- [ ] T002 [DEP] Confirm/file the `django-api` contract issue (beads labels `chrome-ext,django-api`) implementing `contracts/collections-api.md` (`POST /v1/collections/{id}/quotes/` idempotent + `member_collections` on the duplicate-check response). NOT extension code — live/E2E validation (T023) is blocked until this ships; unit tests stub the client.
...
- [ ] T003 [P] Add message + settings types in `src/types/chrome.ts`: `MessageType.ADD_QUOTE_TO_COLLECTION`; `Settings.lastUsedCollectionIds: string[]` (default `[]`).
- [ ] T004 [P] Add API types in `src/types/api.ts`: optional `member_collections?: { id: string; name: string }[]` on the duplicate match; `AddToCollectionRequest { quote_id }` and `AddToCollectionResult { success; alreadyMember?; error? }`.
...
- [ ] T019 [US3] Add the Auto-add toggle + default-collection selector to the overlay dropdown in `src/content/ui/components/account-menu.ts`, reading/writing the SAME settings value via `settings-store` (FR-019/020); populate the selector from the cached collection list. (deps: T005, T008)
- [ ] T020 [US3] Persist the last-used set on each completed add (change-guarded `storage.sync` write) in the add-completion paths of `src/content/ui/overlay-bar.ts`; `seedSelection` now resolves last-used → default → blank precedence (FR-016/017/018). (deps: T013, T018, T005)
...
Task: "T005 settings-store lastUsedCollectionIds (+test)"
