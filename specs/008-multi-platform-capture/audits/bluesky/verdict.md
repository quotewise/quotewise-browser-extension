...
| Focal post selection in threads excludes parent and embedded posts | Pending more scenarios | Original fixture exposed hidden feed posts, proving global `postText` selection is unsafe; live reply/thread fixture still pending |
| Handle trust from the URL is valid for preflight | Pending | No live preflight result committed yet |
| Likes/date visibility is reliable or omitted | Partial candidate | Likes parsed as `37` inside the visible focal root; date is visible only as human text / link aria-label, with no `time[datetime]`, so `postedAt` remains omitted |
...
| Likes | Visible `likeCount-expanded` or `likeBtn` inside the focal root | Medium; one fixture |
