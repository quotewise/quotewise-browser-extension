...
If the poster isn't yet a known Quotewise originator, the extension won't save the quote until the originator is added — and if the same words are already attributed to someone else, it flags the conflict instead of saving a misattribution. You are not guessing; you are recording what the source actually says.
...
**Screenshots** ⚙️ (≥1 required, up to 5) — **1280×800**, full-bleed, 24-bit PNG **no alpha**.
Generated by the maintainers' repeatable pipeline (private tooling, not in this repo):
real extension UI on real posts, signed-in persistent profile. Planned slots:
1. Hero — overlay open, originator resolved, Submit enabled
2. Duplicate check — already-collected state
3. Collections picker during capture
4. Attribution guard (Conflict / MissingOriginator state) — needs a suitable target post
5. Four-platform shot or the saved quote on quotewise.io

**Small promo tile** ✅ — **440×280**, 24-bit PNG no alpha — generated by the maintainers'
tile pipeline.

**Marquee promo tile** ✅ — **1400×560**, generated by the maintainers' tile pipeline.
Optional (enables marquee featuring).
...
| Screenshots (≥1, 1280×800) | ⚙️ maintainer pipeline (needs one-time signed-in profile) |
| Small promo tile 440×280 | ✅ generated |
| Marquee tile 1400×560 | ✅ generated |
